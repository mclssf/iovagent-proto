...
function createSkills(): ManagedSkill[] {
  return skillSeed.map((skill, index) => ({
    ...
    group: index === 19 ? '定制 Skill 组' : [5, 6, 7, 8, 9, 12].includes(index) ? '基础 Skill 组' : '扩展 Skill 组',
  }));
}
...
export const useAgentOpsStore = defineStore('agentOps', () => {
  ...
  const agentCallableSkills = computed<AgentCallableSkill[]>(() => [
    ...
    ...dataEmployeeSkills.value.map((skill) => ({
      ...
      enabled: true, privateToolIds: skill.privateToolIds, group: skill.group,
  ...
  function resolveCustomerAgent(customerId: string, agentId: string): AgentLoadConfig {
  ...
  function saveCustomerAgents(customerId: string, configs: CustomerAgentBinding[]) {
    ...
    const reports = customerReports.value[customerId];
    if (reports) Object.keys(reports).filter((id) => !next.some((binding) => binding.agentId === id)).forEach((id) => delete reports[id]);
  }
  function detectCustomerAgentConflicts(customerId: string, agentId: string) {
    const report = analyzeAgentConflicts(resolveCustomerAgent(customerId, agentId), { skills: availableSkillsForAgent(agentId), tools: tools.value });
    (customerReports.value[customerId] ??= {})[agentId] = report;
    return report;
  }
