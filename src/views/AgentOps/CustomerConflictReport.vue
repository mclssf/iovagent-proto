...
<template>
  <AppDialog :model-value="open" @update:model-value="emit('close')" title="调用冲突检测结果" width="1060px" class="ops-tool-dialog agent-report-dialog">
    <div v-if="report" class="agent-report">
      ...
      <p class="agent-report-explainer">展开所有加载路径，比较 Skill、内置API工具及 MCP 方法的功能描述和已声明的输入、输出 Schema。Skill 与其私有工具的执行依赖、多个 Skill 共享同一私有工具，本身不判定为冲突。</p>
      ...
      <details class="agent-coverage" :open="!report.findings.length"><summary>检测覆盖与限制 · {{ report.schemaCount }} 个工具已检查 Schema，{{ report.coverage.length }} 项说明</summary><ul><li v-for="note in report.coverage" :key="note">{{ note }}</li><li v-if="!report.coverage.length">所有已加载工具均提供输入、输出参数定义。</li></ul></details>
    </div>
