...
const emit = defineEmits<{ editSkill: [skill: ManagedSkill]; editAgent: [id: string] }>();
...
function configureAgent(id: string) {
  detailOpen.value = false;
  emit('editAgent', id);
}
...
<template>
  <section class="tool-management flex h-full min-h-0 flex-col overflow-hidden rounded-md border border-[#deded9] bg-white">
    <div class="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-[#e2e2dc] px-5 py-4">
      <div><h2 class="text-base font-semibold">Tool 管理</h2><p class="mt-1 text-xs leading-5 text-slate-500">{{ tab === 'mcp' ? '管理 MCP 服务连接，查看 Agent 和 Skill 加载关系。' : '同步工程中定义的代码工具，查看属性和加载关系。工具定义只读。' }}</p></div>
    ...
    <div class="flex shrink-0 flex-wrap items-center gap-3 border-b border-[#e2e2dc] px-5 py-3">
      <label class="relative w-full min-w-0 sm:max-w-[360px] sm:flex-1"><Icon :svg="strokeIconPaths.search" :size="15" svg-class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" /><input v-model="search" class="ops-input ops-search" aria-label="搜索 Tool" placeholder="搜索名称、描述或关联 Skill" /></label>
    ...
    <div class="min-h-0 flex-1 overflow-auto">
      <table class="tool-table min-w-[850px]">
        <thead><tr><th class="w-[27%]">{{ tab === 'mcp' ? 'MCP 服务 / Description' : '工具名称 / Description' }}</th><th class="w-[25%]">{{ tab === 'mcp' ? '服务属性' : '输入 / 输出参数' }}</th><th class="w-[14%]">加载方式</th><th class="w-[25%]">关联的 Agent / Skill</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="tool in filteredTools" :key="tool.id">
            ...
            <td>
              <div v-if="store.agentsForTool(tool.id).length"><p class="mb-2 text-xs text-slate-500">直接加载的 Agent</p><div class="flex flex-wrap gap-1.5"><button v-for="agent in store.agentsForTool(tool.id)" :key="agent.id" type="button" class="skill-link" @click="configureAgent(agent.id)">{{ agent.name }}</button></div></div>
              <div v-if="store.skillsForTool(tool.id).length" :class="{ 'mt-3': store.agentsForTool(tool.id).length }"><p class="mb-2 text-xs text-slate-500">私有加载的 Skill</p><div class="flex flex-wrap gap-1.5"><button v-for="skill in store.skillsForTool(tool.id)" :key="skill.id" type="button" class="skill-link" @click="configureSkill(skill)">{{ skill.name }}<span v-if="!skill.enabled" class="text-slate-500">（已禁用）</span></button></div></div>
    ...
    <footer class="border-t border-[#e2e2dc] bg-[#fbfbfa] px-5 py-3 text-xs leading-5 text-slate-500">Tool 由 Agent 直接指定加载，或随已加载的 Skill 私有加载。点击关联名称进入对应配置；同一 Agent 的重复调用路径可通过冲突检测查看。</footer>
    ...
    <ElDialog v-model="detailOpen" :title="selectedTool?.name" width="960px" top="5vh" class="ops-tool-dialog" :close-on-click-modal="false">
      <div v-if="selectedTool" class="space-y-6 text-slate-700">
        ...
        <section><h3 class="mb-3 font-semibold">直接加载的 Agent（{{ store.agentsForTool(selectedTool.id).length }}）</h3><div class="flex flex-wrap gap-2"><button v-for="agent in store.agentsForTool(selectedTool.id)" :key="agent.id" class="skill-link" type="button" @click="configureAgent(agent.id)">{{ agent.name }}</button></div><p v-if="!store.agentsForTool(selectedTool.id).length" class="text-xs text-slate-500">暂无 Agent 直接加载，可在 Agent 配置页中添加。</p></section>
